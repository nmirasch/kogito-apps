...
public abstract class AbstractGraphQLRuntimesQueriesIT extends AbstractIndexingIT {
    ...
    void testProcessInstanceNodes() {
        ...
        checkOkResponse("{ \"query\" : \"{ GetProcessInstanceNodes ( id: \\\"" + processInstanceId + "\\\") {id}}\"}");
    ...
    @Test
    void testProcessInstanceAvailableNodes() {
        String processId = "travels";
        String processInstanceId = UUID.randomUUID().toString();
        KogitoProcessCloudEvent startEvent = getProcessCloudEvent(processId, processInstanceId, ACTIVE, null, null, null);
        indexProcessCloudEvent(startEvent);

        checkOkResponse("{ \"query\" : \"query { ProcessInstances (where: { id: {equal: \\\"" + processInstanceId + "\\\"}}) {availableNodes { id }} }\" }");
        verify(dataIndexApiClient).getProcessInstanceAvailableNodes(eq("http://localhost:8080"),
                eq(getProcessInstance(processId, processInstanceId, 1, null, null)));
    }
